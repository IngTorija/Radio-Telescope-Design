    ...
    def TimeInterval(self,event):                                               #Método para definir la velocidad de muestreo de los datos
        if self.time_unit == "null": 
            ...
            if self.x_counter<130:
                self.serial_device.write("a\n".encode())                                   #Se envía el caracter al dispositivo serial
                tmp = self.serial_device.readline()                                        #Se lee el dato que regresa el dispositivo
                self.y = np.append(self.y,int(tmp))                                     #Se agrega el valor al eje coordenado y
            ...
            if self.x_counter<= 1200:
                self.serial_device.write("a\n".encode())                                   #Se envía el caracter a al dispositivo serial
                tmp = self.serial_device.readline()                                        #Se lee el dato que regresa el dispositivo
                self.y = np.append(self.y,int(tmp))                                     #Se agrega el valor al eje coordenado y
